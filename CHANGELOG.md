...
- **Eighteen ceilings a host hardens with now read the environment, and every one of them is tested before it is trusted.** They were hard-coded, so the only way to move one was `vendor:publish` — and publishing freezes the whole file: every other key in it keeps the default it had that day, including the security defaults a later release tightens. The host who cared enough to lower one upload cap was exactly the host who stopped receiving the others. The new variables are the five field lengths, the metadata value length, the five attachment caps, the screenshot byte cap, the two per-actor rate limits, the time trap, the webhook timeout and the orphan-attachment age; `VISUAL_FEEDBACK_MAIL_SUBJECT_EXCERPT` already existed and joins them in form. Each is validated rather than cast, which is the half with teeth: `(int) 'abc'` is `0`, and for a ceiling zero reads as either *no limit* or *nothing allowed* depending on which one it is — so a typo would have moved the policy silently, and in the loosening direction. An unreadable value now falls back to the shipped ceiling. Where `0` is a real setting it is still accepted, and the two groups are distinguished deliberately: `0` switches a rate limit or the time trap off, while a `0` webhook timeout means *wait forever* to most clients and a `0` orphan age deletes an upload a queue retry is still on its way to use, so those refuse it and keep the shipped value.

  **Upgrade:** nothing to do. With no variable set, every one of the eighteen answers exactly what it answered before — asserted key by key rather than assumed.

### Changed

- **`pushery/legal-consent-for-laravel` 0.33 is accepted.** The range only reached 0.32, so an application on the newer release could not install this package alongside it. Nothing here calls anything that changed; the range simply caught up.
...
- **The widget's mount props travel sealed, so the last four locks are gone and no page load fails for a lock any more.** `availableCategories`, `context`, `fields` and `withScreenshot` were `#[Locked]`, and that attribute throws during hydration — before any method of this package runs, on a widget that lives in the host's layout and therefore survives every `wire:navigate` transition. One consuming application measured **80** of those page errors across three of these names. What a lock was really standing in for is a question about ORIGIN: did this value come from the Blade tag, or from the browser? `mount()` runs once and the tag is never evaluated again, so nothing in Livewire can answer it — unless the answer travels along. It does now: the mount values are encrypted with the application key into one property and the four are restored from it on every request, so a client write is undone rather than refused. Encrypted rather than signed, which also closes the half the lock never covered — a locked value still traveled to the browser in the snapshot, readable by anyone who looked. A copy rather than a fingerprint, deliberately: a digest detects tampering and cannot undo it, the fallback would have to be the configuration, and a client could then tamper on purpose to pull a page onto the wider defaults. The copy is bound to the widget it belongs to, which was **not** true of the first version of this and was found by probing rather than by reading: without that binding a seal is portable between widgets of the same application, so a page offering one category could be handed the seal of a page offering two and accept the wider list — at the validation seam, not merely in the select. There are three cases that fall back to the configuration — a copy that will not open, a copy belonging to another widget, and a copy that does not hold the shape this widget sealed — and in all three it is right: nothing trustworthy is left. The third is a refusal rather than a repair, and that is a correction the coverage floor forced: the first version rebuilt the arrays row by row and skipped what was malformed, which made the skip a line no run could enter, because the mount already filters those rows. Refusing the whole copy is one branch, reachable, and says the stronger thing — a copy that is not what we sealed is not partially used. It still does not throw.

  **Upgrade:** nothing to do. Every per-instance prop keeps its meaning and its precedence, the validation seam still narrows to the categories a page offers, and a page that shaped its form keeps that shape. What changes is only what happens when a browser sends one of those values back: it used to be a 419, and it is now the value the page was mounted with.

- **The widget's `recipient` no longer answers ordinary navigation with a 419, and the address it may name is now declared rather than trusted.** It was `#[Locked]`, and a lock throws during hydration — before any method of this package runs, so nothing can catch it. A consumer measured **52** of those on this one property. But the lock was also the only thing keeping a browser from rewriting the delivery address, so removing it alone would have turned a page load error into an open relay. The permitted set therefore moved into configuration: `mail.to` is always allowed, `mail.allowed_recipients` names any others, and an address that is on neither is answered with `mail.to` — whatever arrives in the snapshot. Checked on the way in and on every request after, so a client can write what it likes and still not move one report — and a host that later narrows the list has the narrowing take effect on the next request, not only on the next mount. The `MailChannel` comment that described the old guarantee is corrected in the same change; a comment asserting a property the code no longer has is worse than none.

  **Upgrade:** nothing to do unless you mount the widget with a `recipient`. If you do, list those addresses under `mail.allowed_recipients` in `config/visual-feedback.php` — `mail.to` itself needs no entry. An undeclared address now fails **at the call site** with a message naming the key, so a page wired to an inbox you never declared says so on the first render instead of quietly delivering somewhere else. Nothing else changes: the per-instance capability is intact, and the address validation at the boundary is unchanged.

- **The widget's `mode` no longer answers ordinary navigation with a 419.** It was `#[Locked]`, and a lock throws during HYDRATION — before any method of this package runs, so nothing can catch it. This widget hangs in the host's layout, so it survives every `wire:navigate` transition and the browser sends the unchanged state back; a consumer measured that in production across three separate locked properties, this one among them. The guarantee the lock stood in for is kept and moved to where it can be checked: the value is narrowed to one of the two shapes this widget renders, on the request it arrives in **and** on the request a client writes it, so every reader — the rendered shape, the screenshot default, the floating-button condition, and the `mode` a submitted report records — meets a value this package chose from its own set. **What a visitor can now do is render their own widget in the other shape and have their own report say so.** That is their page and their record of it; a 419 for everybody navigating past is the worse trade by a distance. **The per-instance override is unaffected** — a host placing two widgets on one page still mixes the shapes, which is why the value is narrowed rather than re-derived from the configuration. A tag written with a shape that does not exist now takes the configured default instead of being stored verbatim, which used to render a widget with no trigger and no form.
- **"Done" closes the WireKit panel again, instead of putting the reporter back in an empty form.** The handler dispatched `wirekit-modal-hide`, an event WireKit has never defined — so the dispatch landed nowhere while the reset beside it ran normally, and the panel that stayed open re-rendered the blank form. It read as a deliberate jump back rather than a dead button, which is why it took a consumer to report it. The event is `wirekit-modal-close`, the name WireKit's own modal component declares. Only the WireKit variant could have this: the plain tree calls `dialog.close()`, a DOM call that cannot fail on a name. A new arm reads the accepted names out of the installed WireKit rather than out of a string written beside the test — one comparing the widget against its own copy of the name would have been just as green over the broken one.
- **The "retake" button under the capture status is no longer flush against the box above it, in either tree.** It was a bare sibling with no class at all, so no rule could reach it; the rule that spaces the other bare buttons listed three classes and its comment said three, and there were four. Both trees were affected — the report came from the WireKit variant, and the plain tree spaces buttons by element and attribute in a way that reached `button[type="submit"]` and not this one. The arm added with it does not take a fourth name: it derives the buttons from the rendered markup — the capture block's button children that follow content — and asks each for a class carrying a vertical margin, in both trees. A fifth one is covered without the list being right.
- **The WireKit trigger follows a host lift again.** WireKit v2.53.0 gave `.wk-fab` a term a page can set to push every floating button clear of something of its own, and this package restates that rule to keep the trigger's two insets equal on older releases. Being two classes against one, and inline in the document, the restatement **wins** — so a host that set the lift moved every floating button except this one, which is the one they were setting it for. The term is carried now, with the kit's own default, so on a release that has no such property the fallback applies and the rendering is byte for byte what it was. **The restatement itself stays**, and deleting it is still the wrong reading: every WireKit from 2.21 to 2.48 needs it, this package's support claim reaches back that far, and a suite resolving the newest allowed release would never have gone red. What was wrong is that a restatement of somebody else's formula stopped growing with it — so there is now an arm that reads the terms out of the **installed** kit's own rule and requires this one to carry all of them, with a control so an unreadable kit cannot pass it by comparing nothing. The next term arrives as a red test rather than as a consumer report.

  **Upgrade:** nothing to do. A page that sets no lift renders exactly as before.

- **A published config no longer hides settings this package adds later.** `vendor:publish` writes every top-level block, and the framework's flat merge then treats each of those blocks as complete — so a key added inside one of them in a later release never reached a host who had published, and read as null with nothing logged. The provider now merges underneath a published file section by section. A list a host narrowed is left exactly as written, including an emptied one: an upload allowlist that quietly regained file types somebody removed would be a worse bug than the one this fixes.

  **Upgrade:** look once at your published `config/visual-feedback.php`, if you have one. Settings added after you published stop reading as null and start reading as whatever this package ships — which is the value you would have got had you published today, and the reason this is a fix. The one case worth a glance is a default that is deliberately strict: it was effectively off for you and is now on. If you relied on that, write it explicitly. A list you narrowed is untouched, including an emptied one, and a cached config still merges nothing at all.
